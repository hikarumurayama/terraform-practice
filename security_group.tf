...
resource "aws_security_group" "app_sg" {
  ...
  description = "app front role security group"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group_rule" "app_out_http" {
  ...
  from_port         = 80
  to_port           = 80
  prefix_list_ids   = [data.aws_prefix_list.s3_pl.id] # S3のプレフィックスリストを指定する
}
...
resource "aws_security_group_rule" "db_in_tcp3306" {
  ...
  from_port                = 3306
  to_port                  = 3306
  source_security_group_id = aws_security_group.app_sg.id # APPサーバーからの接続を許可する
}
